...
interface RiskDistributionChartProps {
    // Accepts both normalized {level, count} and Prisma-raw {riskLevel, _count}
    data: Array<{ level?: string; riskLevel?: string; count?: number; _count?: number }>;
}
...
const RISK_ORDER = ['LOW', 'HIGH', 'CRITICAL', 'MEDIUM'];
    ...
    if (!data || data.length === 0) {
        return (
            <div className="bg-white border border-[#D4C8FF]/50 shadow-sm h-[513px] flex items-center justify-center">
                <p className="text-[15px] font-semibold text-[#A5AEB7]">No risk data available</p>
            </div>
        );
    }

    const chartData = RISK_ORDER
        .map(level => {
            const found = data.find(d => (d.level || d.riskLevel) === level);
            const value = found ? (found.count ?? found._count ?? 0) : 0;
            return { name: level, value, color: RISK_COLORS[level] || '#A5AEB7' };
        })
        .filter(d => d.value > 0);
        ...
        <div className="bg-white border border-[#D4C8FF]/50 shadow-sm p-[24px] h-[513px] flex flex-col">
            ...
            <div className="mb-[16px] flex-shrink-0">
                <div className="flex items-center justify-between">
                    <h3 className="text-[15px] font-semibold text-[#1E1B39] leading-[19px]">Risk Level Distribution</h3>
                    <span className="bg-[#E0E0F6] text-[#6466FF] text-[10px] font-semibold leading-[13px] px-[6px] py-[2px] rounded-full">LAST 7 DAYS</span>
                </div>
                <p className="text-[12px] font-medium text-[#A5AEB7] mt-[6px] leading-[15px]">Breakdown of events by risk severity level</p>
            ...
            <div className="flex-1 flex items-center justify-center">
                <PieChart width={500} height={350}>
                    <Pie
                        ...
                        strokeWidth={2}
                        stroke="#FFFFFF"
                        onClick={(entry) => router.push(`/dashboard/events?riskLevel=${entry.name}`)}
